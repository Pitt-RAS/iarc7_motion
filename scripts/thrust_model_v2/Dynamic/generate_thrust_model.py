...
def generate_2d_polynomial_terms(m_terms, m_degree, n_terms, n_degree):
    ...
    terms = np.empty(rows * (m_degree + 1) * (n_degree + 1)).reshape(
        rows, (m_degree + 1) * (n_degree + 1))

    for row in range(0, rows):
        for i in range(0, m_degree + 1):
            for j in range(0, n_degree + 1):
                terms[row, (i * (n_degree + 1)) + j] = m_terms[
                    row]**i * n_terms[row]**j
...
class Ramp(object):
    ...
    def get_fft(self):
        ...
        frequencies = fftpack.fftshift(
            fftpack.fftfreq(power.size, d=1.0 / self.LOAD_CELL_SAMPLE_FREQ))
    ...
    def calculate_time_constant(self):
        ...
        for i in range(0, max_samples):
            sample_time = self.transition_start_time + \
                i * self.TIME_CONSTANT_CALCULATION_RESOLUTION
        ...
        self.time_constant = (
            self.rise_end_time - self.rise_start_time) / 2.197
        self.fitted_transition_start_time = self.rise_start_time + \
            self.time_constant * np.log(0.9)
        self.response_lag = self.fitted_transition_start_time - \
...
def calculate_response_lags(ramps):
    ...
    print('Average response lag: {} med: {} std dev: {}'.format(
        np.average(response_lags), response_lag, np.std(response_lags)))
...
def create_voltage_to_thrust(ramps, settings):
    thrusts = [ramp.start_thrust for ramp in ramps] + \
        [ramp.end_thrust for ramp in ramps]
    voltages = [ramp.start_voltage for ramp in ramps] + \
        [ramp.end_voltage for ramp in ramps]
    ...
    plt.figure()
    plt.subplot(211)
    plt.plot(voltages, thrusts, 'bo', fit_voltages,
             voltage_to_thrust(fit_voltages), 'r-')

    plt.subplot(212)
    plt.plot(thrusts, voltages, 'bo', fit_thrusts,
             thrust_to_voltage(fit_thrusts), 'r-')
...
def generate_model_map(voltage_to_thrust, time_constant_function, settings):
    ...
    thrust_points = np.linspace(
        thrust_min, thrust_max, thrust_points_count, endpoint=True)

    voltage_points = np.linspace(
        voltage_min, voltage_max, voltage_points_count, endpoint=True)

    voltage_points_mesh, thrust_points_mesh = np.meshgrid(
        voltage_points, thrust_points)
    ...
    T = (1 - np.exp(-Ts / time_constant_function(thrust_points_mesh, voltage_points_mesh))) * \
        (voltage_to_thrust(voltage_points_mesh)
         - thrust_points_mesh) + thrust_points_mesh
...
if __name__ == "__main__":
    ...
    voltage_to_thrust_poly, thrust_to_voltage_poly = create_voltage_to_thrust(
        ramps, settings['voltage_to_thrust_estimator'])
    thrust_voltage_to_time_constant = create_time_constant_to_last_thrust_and_voltage(
