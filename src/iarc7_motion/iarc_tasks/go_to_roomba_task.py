...
class GoToRoombaTask(AbstractTask):
    ...
    def __init__(self, task_request):
        ...
        self.roomba_id = task_request.frame_id + '/base_link'

        if self.roomba_id is None:
        ...
        rospy.loginfo("Let us see the truth")
        rospy.loginfo("The _z_position: %f",self._z_position)
        rospy.loginfo("The _MIN_MANEUVER_HEIGHT: %f",self._MIN_MANEUVER_HEIGHT)
    ...
    def get_desired_command(self):
        with self._lock:
            ...
            if self._state == GoToRoombaState.translate:
                ...
                # uses the _roomba_point to see if the roomba is out of range
                if not self._check_max_roomba():
                    return TaksAborted(msg='The provided roomba is not close enough to the quad')
                ...
                y_vel_target = (self._roomba_point_point.y * self._K_Y + roomba_y_velocity)
                ...
                self._path_holder = reinitialize_translate_stop_planner(x_vel_target,y_vel_target,self._z_position)
                """ quarantine end"""
